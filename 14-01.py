...
def main():
    ...
    cave_representation.update({
        ...
        "ymin": 0,
        "ymax": max(rock_coordinates, key=lambda p: p[1])[1],
    })
...
def get_falling_position(sand, cave_representation):
    ...
    while sand != previous_position:
        ...
        if not is_in_cave(sand, cave_representation):
            return None
...
def is_in_cave(sand, cave_representation):
    ...
    sandx, sandy = sand
    return (xmin <= sandx <= xmax) and (ymin <= sandy <= ymax)
...
def draw_stuff(cave_representation):
    ...
    for y in range(ymin, ymax + 1):
        print("".join([rep.get((x, y), ".") for x in range(xmin, xmax + 1)]))
    time.sleep(0.1)
